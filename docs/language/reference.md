...
[Versioning & releases](../project/releases.md) is the authoritative list, and
...
* **Source** is UTF-8 text; `#` starts a comment running to end of line. Space,
  ...
  have no terminator, so mind the `return` caveat in [Functions & errors](functions-errors.md).
...
renames and permutes them per build (see [Polymorphic artifacts](../execution/artifacts.md)),
...
* [Language basics](basics.md) — values, syntax, operators, control
  flow and output.
* [Functions & errors](functions-errors.md) — closures, the error
  model and the safety budgets.
* [Standard library](standard-library.md) — every built-in and
  runtime namespace.
* [Polymorphic artifacts](../execution/artifacts.md) — how the opcode names and
