...
[Functions & errors](functions-errors.md) for a complete example.
...
* `return` swallows the next expression because newlines do not terminate a
  ...
  See [Functions & errors](functions-errors.md) for the
  demonstration.
* There is no `**`, no bitwise/shift operators and no compound assignment.

## Related pages

* [Functions & errors](functions-errors.md) — `fn`, closures, the
  error model and the step/wall-clock budgets.
* [Reference](reference.md) — grammar, precedence table, semantics
  table and the bytecode instruction set.
* [Standard library](standard-library.md) — the built-in functions
  used above (`print`, `len`, `range`, `transform`, `json_encode`).
* [CLI reference](../operations/cli.md) — `run`, `exec`, `build`, `disasm`.
