...
[collectors](../runtime/collectors.md)). The first two columns are the actual
...
* **Written to disk:** one file, the artifact, whose bytes carry no plaintext of
  the program (see [polymorphic artifacts](artifacts.md)).
...
[fileless execution](fileless.md).
