...
* **Correlation by behaviour.** Two builds that read the same files, open the
  ...
  [execution modes](modes.md) for the measured equivalence).

## Related

* [Execution modes](modes.md) — where each mode keeps its program
  text, and the measured file/process telemetry.
* [Fileless execution](fileless.md) — the payload as an anonymous
  memory file instead of an artifact.
* [CLI reference](../operations/cli.md) — every flag of `build`, `exec` and
