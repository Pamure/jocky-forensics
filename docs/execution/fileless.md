...
See [execution modes](modes.md) for the measured telemetry each
mode produces.
...
* [Execution modes](modes.md) — the three modes side by side, with
  process-image and telemetry measurements.
* [Polymorphic artifacts](artifacts.md) — what the payload can be
  before it is handed to the fileless path.
* [Detection checks](../runtime/detection.md) — the checks that find this
