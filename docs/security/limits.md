...
* **A scan can be blind without saying so loudly.** `partial_visibility` fires
  ...
  [Threat model](threat-model.md); the section is there because the mitigation
...
| Ingress selection | `--sni`/`--host-header` pick a vhost at an ingress you control; this is **not** domain fronting, which every tier-1 CDN closed between 2018 and 2024 | [Server & agents](../operations/management.md) |

## Where the limits point

The same runtime ships the detector for every technique it uses
([Detection checks](../runtime/detection.md)), the fileless mechanics are described with their
telemetry in [Fileless execution](../execution/fileless.md), the artifact format and its honest
limitation in [Polymorphic artifacts](../execution/artifacts.md), and the evidence that a reviewer can
re-check in [Evidence harness](../operations/evidence.md). What is not fixed yet is listed with
effort and rationale in [Roadmap](../project/roadmap.md); what an attacker can do to the
