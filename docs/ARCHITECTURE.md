...
[Detection checks](runtime/detection.md) page is generated from it.
...
[Polymorphic artifacts](execution/artifacts.md) for the transform table and
...
[Server & agents](operations/management.md).
...
measurement, not a specification. [Evidence harness](operations/evidence.md)
explains each stage and how to re-run it; [Honest limits](security/limits.md)
...
- [Collectors](runtime/collectors.md) — what each namespace returns.
- [Execution modes](execution/modes.md) — choosing between `run`, `exec` and `fileless`.
- [Polymorphic artifacts](execution/artifacts.md) — the artifact format in detail.
- [Contributing](../CONTRIBUTING.md) — extension points and test layout.
