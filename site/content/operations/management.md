...
later. Everything in this page was run against a real server on this host; the
request and response bodies are quoted as they came back.

The design rule is that **all policy lives on the server** — which hosts, which
checks, which payload, when. The agent is deliberately dumb: it runs exactly what
it is handed, and journals what it did before it reports it.
...
On first start the certificate is generated by shelling out to `openssl`, so the
...
Case data is not world-readable: the state directory is `0700` and both
`store.db` and `agent.key` are `0600` (the certificate is `0644`):
...
certificate is written into the state directory at all — verified by starting a
second server on that pair: only `store.db` appeared there.
...
Token handling is strict where it matters and convenient elsewhere. A missing
token fails before any connection is attempted:
...
`--insecure` is an explicit opt-out and says so:

```text
jocky: WARNING --insecure disables certificate pinning; the token is exposed to anyone who answers that socket
```
...
That value is the lowercase-hex SHA-256 of the DER certificate;
`openssl s_client -connect HOST:PORT </dev/null | openssl x509 -noout -fingerprint -sha256`
prints the same fingerprint with colons and uppercase, so it can be compared by
...
The same for the fileless kind:

```bash
cat > /tmp/jky-docs/probe.jky <<'JKY'
# Same shape, executed in fileless mode on the target.
emit {
  "severity": "info",
  "check": "memory",
  "title": "fileless host probe",
  "evidence": {"listeners": len(net.listeners())}
}
JKY
...
in the same instant cannot both win, and the poll response carries the digest of
the payload so a client can check it before executing:
...
The bundled agent does not currently re-check that digest client-side; the value
is stored with the job and echoed back on the result so both sides can be
reconciled afterwards.

Submitting any other kind is accepted by the server and rejected by the agent at
...
steps per job, plus a 120 s timeout around the fileless child. A runaway payload
is cut off rather than allowed to pin the host.
    ...
    "findings": [
        {
            ...
            "check": "memory",
            "title": "fileless host probe",
            "created_at": "2026-09-15T17:50:14.657+00:00",
            "evidence": {
                "listeners": 19
            }
        },
        {
            ...
            "check": "asset",
            "title": "host asset inventory",
            "created_at": "2026-09-15T17:50:13.864+00:00",
            "evidence": {
                "listeners": 19,
                "processes": 50
            }
...
local journal; the server keeps findings plus provenance. If the process image
must be recorded centrally, emit it from the script.

## HTTP API

Every route answers JSON. Every route except `/v1/health` requires the header
`X-JKY-Token: <token>`, compared with `hmac.compare_digest` so a token guess
cannot be timed. Request bodies must be a JSON object, `Content-Length` is
required, and unknown JSON fields are rejected instead of ignored.
...
| `GET` | `/v1/findings` | — | `200 {"count":N,"findings":[…]}`; query `?limit=1..1000` (default 200), `?severity=…` |
...
The poll/result handshake is where a plausible-looking call could corrupt an
audit trail, so result submission validates in three stages: the agent must be
enrolled (`404`), the job must have been claimed by *that* agent (`409`), and a
job that is already finished is acknowledged as a duplicate without writing
anything (`200`, idempotent). Findings are only written when the job actually
transitions `running → finished`.
...
`jobs.status` is one of `queued`, `running`, `ok`, `error`. Rows from the run
described above (`sha16` is the payload digest, truncated for printing):
...
{'job_id': 'job_f1be3d85fee594e7', 'kind': 'source', 'status': 'error',
 'target': 'agt_b4874e9738633106', 'claimed_by': 'agt_b4874e9738633106',
 'sha16': '0cb86b804a5ac24a', 'started_at': '2026-09-15T17:50:15.186+00:00',
 'finished_at': '2026-09-15T17:50:15.321+00:00'}
```
...
record. Entries are written *before* the result is reported, so the local record
survives an unreachable server. The complete journal from the two jobs above:
...
terminates TLS for it and forwards to the origin — a third party that must
exist, be reachable, and be willing. Nothing about `--sni` creates that, no CDN
is involved in a local run, and none is claimed. Treat the flag as a way to test
a deployment that already has a front, never as a substitute for one. The
reasoning is in `research/cdn_fronting.md`, and the module docstring says the
