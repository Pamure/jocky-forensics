...
* [Language basics](/docs/language/basics) — values, syntax, operators,
  truthiness, `emit` vs `print`.
* [Reference](/docs/language/reference) — the instruction set the compiler
  emits for closures, calls and handlers.
* [Standard library](/docs/language/standard-library) — `transform`, `filter`,
  `sort_by`, `count` and the rest of the built-ins.
* [CLI reference](/docs/operations/cli) — the flags that set the budgets.
